...
:isVanished(false), m_chain(0), m_combo(0)
{
}
                ...
                for (int y_ = 0; y_ < GetLine(); y_++)
                {
                    for (int x_ = 0; x_ < GetColumn(); x_++)
                    {
                        if (puyo_temp_dfs[y_ * GetColumn() + x_])
                            // VanishAnimation();
                            puyo_temp_dfs[y_ * GetColumn() + x_] = NONE;
                            SetValue(y_, x_, NONE); //暫定
                    }
                }
                y = x = 0;//リセットしてコンボ可能かどうか
                // exit_loop_flg = true;
            }
...
{
    if (GetValue(y, x) != prev_puyo_color || puyo_temp_dfs[y * GetColumn() + x] != false) return 0;
    int ret = 1;
...
PuyoControl::PuyoControl() 
    : DELAY_FORCE_MOVEDOWN(1), m_newPuyoLandingCount(0),m_gameOverFlag(false)
{
}
...
{
    if (m_newPuyoLandingCount==1)return;
    //一時的格納場所メモリ確保
